								...
								<xsl:choose>
										...
										<xsl:when test="linkType='0'">
											...
											<xsl:if test="contains(linkUrl, 'boaconsulta')">
										      <xsl:attribute name="onclick">ga('send', 'event', 'link', 'clique', 'homeprestador-link_agenda-consultas');</xsl:attribute>
										    </xsl:if>
										    <xsl:if test="contains(linkUrl, 'prontmed')">
										      <xsl:attribute name="onclick">ga('send', 'event', 'link', 'clique', 'homeprestador-link_prontu�rio-eletronico');</xsl:attribute>
										    </xsl:if>
										</xsl:when>
										<xsl:otherwise>

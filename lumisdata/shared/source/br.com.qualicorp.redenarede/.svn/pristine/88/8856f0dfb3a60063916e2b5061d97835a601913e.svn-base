	        ...
	        <div class="smt-features-carousel smt-carousel-wrapper">
		                    ...
		                    <a  class="smt-link-block">
								<xsl:choose>
									...
									<xsl:when test="linkType='0'">
										...
										<xsl:if test="contains(linkUrl, 'questionariosaude')">
									      <xsl:attribute name="onclick">ga('send', 'event', 'banner', 'clique', 'homebeneficiario-top_banner1');</xsl:attribute>
									    </xsl:if>
									    <xsl:if test="contains(linkUrl, 'busca-na-rede')">
									      <xsl:attribute name="onclick">ga('send', 'event', 'banner', 'clique', 'homebeneficiario-top_banner2');</xsl:attribute>
									    </xsl:if>
										<xsl:if test="contains(linkUrl, 'boaconsulta')">
									      <xsl:attribute name="onclick">ga('send', 'event', 'banner', 'clique', 'homebeneficiario-bottom_banner1');</xsl:attribute>
									    </xsl:if>
									    <xsl:if test="contains(linkUrl, 'mais-servicos')">
									      <xsl:attribute name="onclick">ga('send', 'event', 'banner', 'clique', 'homebeneficiario-bottom_banner2');</xsl:attribute>
									    </xsl:if>
									</xsl:when>
									<xsl:otherwise>
	            ...
	            <xsl:if test="//data/row">
					<div class="smt-section-actions">
                    	<a href="#" class="smt-secondary-button">Mais +</a>
                	</div>
				</xsl:if>
	        </div>
	    </div>

...
function LumisChatMessages(element, getMessagesToModerate, conversationId, chatUserId, serviceInterfaceInstanceId)
{
	...
	function updateUsers(xmlDom)
	{
		...
		{
			if (selects[i].getAttribute('class') != null 
					&& selects[i].getAttribute('class').search(/cLumIsChatUsers/) != -1)
			{
		...
		if(begin != end)
		{
			...
			if(response != null && response != "")
			{
				if(element.firstChild == null)
					...
					element.firstChild.innerHTML = element.firstChild.innerHTML + response;
			}
